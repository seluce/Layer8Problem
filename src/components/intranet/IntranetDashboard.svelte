...
<script>
    ...
    const data = $derived(intranetPages());
    const page = $derived(data?.dashboard?.page ?? null);
...
<div class="max-w-6xl mx-auto mt-8 px-4 grid grid-cols-1 md:grid-cols-3 gap-8">
    ...
    <div class="col-span-2 space-y-8">
        <div class="bg-slate-800 p-8 rounded-xl border border-slate-700 shadow-md">
            <h1 class="text-3xl font-black mb-3 text-white">{page?.welcomeTitle}</h1>
            <p class="text-slate-300 leading-relaxed mb-6">{page?.welcomeText}</p>

            <div class="bg-amber-900/20 border-l-4 border-amber-500 p-4 rounded-sm text-amber-200/90 text-sm">
                <h3 class="font-bold text-amber-400 uppercase tracking-wide mb-1">{page?.facilityTitle}</h3>
                {page?.facilityText}
            </div>
        </div>
        ...
        {#if data?.employee}
            <div class="bg-slate-800 p-8 rounded-xl border border-slate-700 shadow-md">
                <h2 class="text-xl font-bold mb-6 flex items-center gap-2 text-white">{page?.employeeTitle}</h2>
        ...
        <div class="bg-slate-800 p-8 rounded-xl border border-slate-700 shadow-md">
            <h2 class="text-xl font-bold mb-6 flex items-center gap-2 text-white">{page?.feedTitle}</h2>
            <div class="space-y-6">
    ...
    <div class="space-y-8">
        <div class="bg-blue-900/30 border border-blue-800/50 p-6 rounded-xl shadow-lg relative overflow-hidden">
            <h3 class="font-bold mb-4 text-blue-400 uppercase tracking-widest text-xs">{page?.visionTitle}</h3>
            {#if data?.vision_quote}
                <blockquote class="italic text-lg font-serif leading-relaxed mb-4 text-slate-300">"{data.vision_quote}"</blockquote>
            {/if}
            <p class="text-sm font-bold text-slate-400">{page?.visionAuthor}</p>
        </div>

        <div class="bg-slate-800 p-6 rounded-xl border border-slate-700 shadow-md">
            <h3 class="font-bold mb-4 text-white">{page?.statusTitle}</h3>
            ...
            {#if data?.incident}
                <div class="mt-5 pt-4 border-t border-slate-700">
                    <div class="flex justify-between items-center">
                        <span class="text-slate-400 text-sm">{#each page?.incidentLabel ?? [] as line, i (i)}{line}<br>{/each}</span>
        ...
        {#if data?.kpi}
            <div class="bg-slate-800 p-6 rounded-xl border border-slate-700 shadow-md">
                <h3 class="font-bold mb-4 text-white">{page?.kpiTitle}</h3>
                <div class="flex items-baseline justify-between gap-3">
                    <span class="text-slate-400 text-sm">{page?.kpiLabel}</span>
                    <span class="font-black text-3xl text-blue-400">{data.kpi.value}</span>
                </div>
                <div class="flex items-baseline justify-between gap-3 mt-1">
                    <span class="text-slate-500 text-xs uppercase tracking-wider">{page?.kpiTargetLabel}</span>
                    <span class="font-bold text-slate-400">{page?.kpiTarget}</span>
                </div>
