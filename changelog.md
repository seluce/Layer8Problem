...
* It was translated scene by scene, not word by word. Where a punchline does not carry in English, it has been rebuilt - Kevin's nickname for Miller, for instance, or the abbreviation for the dumbest user imaginable. What draws its effect from the German stays German: the Mettigel, the TÜV, the Döner, the colleagues' names.
* Müller is called Miller in English. It is the literal translation and carries the same joke - the most common surname in the country, and therefore nobody in particular. The colleagues keep their names: Kevin, Chantal, Egon, Gabi, Markus, Frau Elster and Dr. Wichtig still work in a German office.
* There are two places to switch: at the bottom of the start screen with a plain DE | EN that you find without reading, and in the settings. The switch on the start screen is deliberate - anyone who ends up with the wrong language cannot go looking for the settings menu, because its label is in the wrong language too.
* On the first start the browser's language decides, and in the Steam version the one set there. Anyone who has played before stays with German - an update that switches loyal players to English would be poor thanks.
* Save files are language-independent. You can switch in the middle of the week without losing anything: reputation, backpack, events already seen and open stories all stay; only the language they are told in changes.
* The switch happens on the spot, without reloading the page and without the question of whether you would like to continue the day. You may switch in the middle of the day, in the middle of an event and in the middle of a phone chat: the screen currently open comes along - title, text and the answers on offer. What had already been said stays as it was said: the log, a result already concluded and the bubbles of a running chat are a memory and not display text, and you do not rewrite a logbook retrospectively.
* The achievements have English names where the name carries the joke: the Faulpelz with the sloth on its badge becomes Sloth, and the Streber becomes a Swot.
* In the Steam version, everyone now reads the friends list in their own language. Until now it showed the player's language - play in German and you were off fetching coffee for your English friends as well. What Steam displays is now decided by whoever is looking.
...
* A phone errand could briefly show a bar above 100 % before it snapped back to 100 %. The value was never really higher - the display was simply a blink ahead of the cap. Now it stops where it belongs.
* Continue a day that was saved before an update and the log showed the version from back then - while the terminal header next to it named the new one. The start line belongs to the running session, not to the save file, and now names the same version in both cases. The rest of the log is untouched: what is once written there is memory.
* The knowledge said of Herr Aluhut that he calls "under changing aliases". He has exactly one, and the punchline of the second call is precisely that he announces himself with it and expects to be recognised. That the number is nevertheless the same every time remains the real joke.
* The knowledge said the boss's office was on the "floor above the server room" - the summary directly beneath it, both notes and the leaking battery unanimously say the opposite. The office is below the server room, which is why everything drips into it.
* Kevin's second call was headed "Praktikant Kevin". Kevin is the apprentice - that is what his card in the team says, and what stands above his five other calls. The call is now simply "Kevin (Wieder)", as the tinfoil hat and the unknown number do it too.
* Kevin had been promoted to intern in three further places - Frau Elster did not want to entrust "diesem Praktikanten" with her cat's data, at the summer party Markus first cornered "Azubi Kevin" and then talked at an intern, and one morning mood was called "Der aufmerksame Praktikant". It now says apprentice everywhere.
* At the bag check by the exit you put a donut on top as "interest" - in the result, Frau Elster then accepted chocolate. She now takes what was actually put in front of her.
* Lose your nerve in front of Sarah from HR and you stammer an error message. Until now it was only the number; now it is the whole sentence, including the line that belongs underneath. What is missing is the answer.
* A full descaling took an hour and a half in the result text, but only seventy-five minutes in the follow-up the next morning - and seventy-five minutes is what it actually costs. Both places now name the same figure.
* Ten minutes in the tea kitchen became, in the same sentence, "the most relaxing quarter of an hour of the day". It stays ten minutes, and they are still the most relaxing.
* At the supply cupboard, Bernd was suddenly in logistics although he sits in sales everywhere else - on his own card in the knowledge, in his seven mails, in the server room where he weeps over his lost revenue, and at the desk that is cleared out after his last day. He stays in sales; the knowledge now says so at the copy room too.
* The thick manual had a different name depending on where you found it: "Win95 Handbuch" in the inventory, "Win95-Handbuch" on the buttons, "Windows 95 Handbuch" when found in the box. It is now Windows-95-Handbuch in all twelve places. The book title under the wobbly table leg stays as it is - "Windows 95 - Das Handbuch" is, after all, the title and not the item.
* Four spelling mistakes in the errands that had been standing since 5.0: at the jammed window a "Taffe" travelled across the table, a "Wasserlasche" helped against the smoking waste-paper basket, in front of Frau Elster you were allowed to "kapitäulieren", and "Flokies" blew into the office. They are now Tasse, Wasserflasche, kapitulieren and Flocken.
* Shout at the boss while his click eats its way through the drive and you used to bellow "STEKCER ZIEHEN, SIE IDIOT!". Even in the heat of the moment the plug is now called Stecker.
...
For developers:
* The four effects have the same names everywhere: m for minutes, l for laziness, a for aggro, b for boss. Before, f and c were German mnemonics (Faulheit, Chef) in an otherwise English data model, and the same three values carried three different names depending on where you found them. Affected are options, fail and results (15,322 keys), the item blocks use and passive - where 5.0 still documented al, fl and cr -, the curve point in statHistory and the week row in weekLog, whose endFl/endAl/endCr are now endL/endA/endB, and peakC is now peakB.
* The time in the curve point is called m, no longer t. In an option, t is the button text, and the same letter with two meanings was precisely the confusion that was meant to disappear.
* The running state deliberately stays state.fl, state.al and state.cr, as do addStat and the display ids val-fl, val-al and val-cr. Single letters on an object that is read right across the engine are harder to read than in a data literal. The boundary is therefore a rule and not a list of exceptions: what you write carries letters, what runs carries fl/al/cr.
* Results additionally understood min, fl, al and cr as second names. The alias came from a time when there were events using it; in the current stock none did any more. It has been dropped without replacement, and the data checker now reports the old names as an unknown field - so an overlooked key turns red rather than quietly turning into zero.
* Save files from 5.0 need nothing. statHistory and weekLog are converted on loading, on both resume paths and without anything being lost: the day curve and the week summary of an interrupted run look the same after the update as before. The save code from the export does not contain these two forms anyway.
